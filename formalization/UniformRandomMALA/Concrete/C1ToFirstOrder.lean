...
and smoothness by a Lipschitz bound on that gradient, as in
`eq:first-order-assumptions`. In particular, no Hessian,
...
Theorem 2.1.5, an external literature reference). Here it is proved by
differentiating along an affine line and
