...
The additional nonconvex assertion in Appendix B (`app:rejection-overlap`) is outside these endpoints:
...
/-- Theorem 2.1 (`thm:main`), non-lazy clause `eq:master-gap`:
the hypotheses use only `C¹`,
...
/-- The lazy clause of Theorem 2.1 (`thm:main`) under the manuscript's first-order
...
/-- Corollary 2.2 (`cor:sqrt-d-endpoint`), first display, for randomized MALA under the manuscript's
...
/-- Corollary 2.2 (`cor:sqrt-d-endpoint`), simplified second display, for randomized MALA under the
...
/-- The displayed threshold `p⋆` in Theorem 2.1 (`thm:main`). -/
def paperMomentThreshold (V : C1Potential d) (A₀ : ℝ) : ℝ :=
  A₀ * (1 + Real.log ((d : ℝ) + 1) + Real.log (V.L / V.m))

/-- The non-lazy right-hand side `eq:master-gap` in Theorem 2.1 (`thm:main`). -/
...
/-- Both clauses of Theorem 2.1 (`thm:main`) with the same universal constants chosen
...
/-- Proposition 3.3 (`prop:separated`) for the normalized first-order target. -/
...
/-- Proposition B.1 (`prop:stationary-rejection`) in real-moment form, for every `p ≥ 1`. This endpoint
...
/-- Proposition 3.2 (`prop:overlap`) under the actual-gradient first-order assumptions. -/
...
/-- Proposition 3.4 (`prop:flow`) in its full parameter range under the manuscript's
